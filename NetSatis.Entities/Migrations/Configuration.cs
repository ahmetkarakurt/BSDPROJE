namespace NetSatis.Entities.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<NetSatis.Entities.Context.NetSatisContext>
    {
        ...
        protected override void Seed(NetSatis.Entities.Context.NetSatisContext context)
        {
            ...
            if (!context.BSD_TBL_Kullanicilar.Any(c => c.KullaniciAdi == "BSD"))
            {
                ...
                new BSD_TBL_RAPORLAR_VW {VwAdi="Satýs iade",VwYolu= @"\DizaynRapor\Stok\AdetliSatisiade.xml",Image= @"\ico\research-and-development.png",Kategorid=anamnu.Where(c=>c.KategoriAdi=="STOK").Select(c=>c.id).FirstOrDefault()},
                new BSD_TBL_RAPORLAR_VW {VwAdi="Satýlan Stok Listesi",VwYolu= @"\DizaynRapor\Stok\AdetliSatisStokListe.xml",Image= @"\ico\products.png",Kategorid=anamnu.Where(c=>c.KategoriAdi=="STOK").Select(c=>c.id).FirstOrDefault()},
                ...
                new BSD_TBL_RAPORLAR_VW {VwAdi="iþ Emri Durumlarý",VwYolu= @"\DizaynRapor\Uretim\IsEmriDurumlari.xml",Image= @"\ico\assembly.png",Kategorid=anamnu.Where(c=>c.KategoriAdi=="URETIM").Select(c=>c.id).FirstOrDefault()},
                new BSD_TBL_RAPORLAR_VW {VwAdi="Üretimde Kullanýlan Ürünler",VwYolu= @"\DizaynRapor\Uretim\UretimdeKullanilanUrunler.xml",Image= @"\ico\products (1).png",Kategorid=anamnu.Where(c=>c.KategoriAdi=="URETIM").Select(c=>c.id).FirstOrDefault()}};
